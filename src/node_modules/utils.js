...
async function uri(id, user, type, {width, height, square}) {
	...
	const res = await api.get(`u/${type}/${id}/${query}`, user && user.token);
	if (res && res.success) {
		return res.data;
	} else {
...
export async function getMedia(type, id, user, { ...options }) {
	...
	if (user && user.token) {
		...
		cached = (_urls = get(urls)) && _urls.has(id) && _urls.get(id)[ stringified ] || false;

		if (!(url = cached)) {
			res = await uri(id, user, TYPE, params)
			if (res) {
				(url = res[ 'url' ]) && urls.add(res)
			} else {
				throw new Error('getMedia failed')
			}
		}
...
export function createRedirectSlug(page) {
    let tab = page.query && page.query.tab;
    let path = page.path;
    let bit = "";
    return path
