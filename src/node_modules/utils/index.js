...
import { formatISO9075, differenceInHours } from 'date-fns';
...
export function timespan(date, endDate) {
  let start = hasStarted(date) ? new Date() : date;
  return Math.abs(differenceInHours(start, endDate));
}
export function readoutDuration(date, endDate) {
  if (endDate && isExpired(endDate)) return 'abgelaufen';

  let time = timespan(date, endDate);
  let days = Math.floor(time / 24);
  let hours = time % 24;
  let readout = `${days ? days : ''}${days ? ' d ' : ''}${hours ? hours : ''}${hours ? ' hrs ' : ''}`;
