...
export function redirectPath(page, session = {}) {
  ...
  if (page.query.redirect) return page.query.redirect;
  if (path === 'users' && session.user) {
    path = path.concat('/', session.user.id);
  }
  return path.concat(parseQueryString(page.query, ['token', 'redirect']));
}
function parseQueryString(query, exclude) {
  let concated = '?';
  for (let key in query) {
    exclude.indexOf(key) < 0 && (concated = concated.concat(`${key}=${query[key]}&`));
  }
