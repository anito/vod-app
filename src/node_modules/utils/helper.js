...
export async function extendSession() {
  let res;
  try {
    // read lifetime from store and write to cookie and session store
...
export function createRedirectSlug(page) {
  ...
  return `?redirect=${path}${parseQuery(page.query)}`;
}

export function redirectPath(page, session = {}) {
  
  if (page.query.redirect) {
    return page.query.redirect;
  } else {
    let path = "";
    path = path.concat(session.role === "Administrator" ? `/users/${session.user && session.user.id}` : "/videos");
    return path.concat(parseQuery(page.query));
  }
}
...
export function parseQuery(query) {
  ...
  for (let key in query) {
    exclude.indexOf(key) < 0 &&
      (concatenated = concatenated.concat(`${key}=${query[key]}&`));
  }
