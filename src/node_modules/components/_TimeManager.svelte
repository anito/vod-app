<script>
  ...
  function openScheduleDialoge(video) {
    dialogVideoId = video.id;
    scheduleDialoge.open();
  }
  function openRemoveDialoge(e, video) {
    e.stopPropagation();
    dialogVideoId = video.id;
  ...
  async function removeDialogeCloseHandler(e) {
  ...
  function scheduleDialogeCloseHandler(e) {
...
<div
  ...
  <Component>
    <div slot="header">
      <Header h="3">Gebuchte Kurse</Header>
    </div>
    <List
      class="video-list"
      threeLine
            ...
            <SimpleVideoCard
              ...
              {#if 'Administrator' === $session.role}
                <IconButton
                  ...
                  on:click={(e) => openRemoveDialoge(e, video)}>
...
{:else}
  <div class="grid-item videos" class:no-videos={!noneUserVideos.length}>
    <Component>
      <div slot="header">
        <Header h="3">Weitere Kurse</Header>
      </div>
          ...
          {#each noneUserVideos as video (video.id)}
            <SimpleVideoCard
              ...
              <IconButton
                ...
                color="primary"
                on:click={() => openScheduleDialoge(video)}>
...
{/if}
<Dialog
  bind:this={scheduleDialoge}
  aria-labelledby="list-selection-title"
  aria-describedby="list-selection-content"
  on:MDCDialog:closed={scheduleDialogeCloseHandler}>
...
<Dialog
  bind:this={removeDialoge}
  aria-labelledby="event-title"
  aria-describedby="event-content"
  on:MDCDialog:closed={removeDialogeCloseHandler}>
  <Title id="event-title">
    {`${(dialogVideo && dialogVideo.title) || '--'} ist im Moment aktiv`}
  </Title>
  <Content id="event-content">
    Dieser Kurs verfügt über ein aktives Zeitfenster. Den gebuchten Kurs
    trotzdem entfernen?
  </Content>
  <Actions>
    <Button action="none">
