<script>
  ...
  import { videos, videosAll, users, flash, infos } from "stores";
  ...
  import { toISODate, proxyEvent, sortByEndDate, sortByTitle } from "utils";
  ...
  $: userVideos = ((idx) => {
    if ($session.role === ADMIN) {
      ...
      return $users[idx]?.videos
        .filter((v) => {
          return (
            new Date(v._joinData.start) <= new Date() &&
            new Date(v._joinData.end) >= new Date()
          );
        })
        .sort(sortByEndDate);
    }
