<script>
  ...
  async function saveTime(id, start, end) {
    ...
    if (res.success) {
      handleSuccess(res, $_('text.period-updated'));
    } else {
      handleError(res);
    }
  }
...
>
  <Title id="list-selection-title">{$_('text.select-period')}</Title>
...
>
  <Title id="event-title">
    {$_('text.video-is-active', { values: { title: schedulingVideoTitle } })}
  </Title>
