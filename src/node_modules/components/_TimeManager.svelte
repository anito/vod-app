<script>
  ...
  async function saveTime(id, start, end) {
    ...
    if (res.success) {
      handleSuccess(res, $_('text.timeframe-updated'));
    } else {
      handleError(res);
    }
  }
      ...
      {#if 'Administrator' !== currentRole}
        ...
        {:else if currentUser}
          <div class="flex flex-1 flex-col self-center text-center">
            <div class="my-10">
              {@html `Benutzer <strong>${username}</strong> hat keine Videokurse abonniert`}
            </div>
          </div>
        {:else}
          <div class="flex flex-1 flex-col self-center text-center">
  ...
  <div class="grid-item time">
    <Component>
      ...
      <DateRangePicker
        ...
        customHeaderHeight
        class="sm-header"
        locale={locales[$locale.slice(0, 2)]}
        {firstDayOfWeek}
        {startDate}
        {endDate}
        {maxDate}
...
{:else}
  <div class="grid-item videos" class:no-videos={!noneUserVideos.length}>
    <Component>
      <div slot="header">
        <Header mdc h="5">
          <div class="uppercase" style="font-weight: 400; text-align: right;">{$_('text.more-courses')}</div></Header
        >
      </div>
    ...
    <List radioList>
      ...
      {#if !currentUser.token || isExpired || !currentUser.active}
        <div class="mt-3">
          ...
          <div>
            Der Account des Benutzers <strong>{username}</strong> ist inaktiv. Über den Tab<i>Benutzerdaten</i>
            können Sie so die folgenden Probleme beheben:
            <div>
              <ul class="reasons-list">
                {#if !currentUser.token}
                  <li>Der Account verfügt über keinen Token.</li>
                {/if}
                {#if isExpired}
                  <li>Der aktuelle Token ist abgelaufen.</li>
                {/if}
