<script>
  ...
  import List, { Item, Graphic, Separator, Text } from '@smui/list/styled';
  ...
  import { DateRangePicker, Header } from '@anito/site-kit/';
  import Component from './_Component.svelte';
  import { startOfDay, endOfDay, parseISO } from 'date-fns';
  import * as locales from 'date-fns/locale';
  import { toISODate, proxyEvent } from 'utils';
  import Dialog, { Title, Content, Actions, InitialFocus } from '@smui/dialog/styled';
  ...
  $: noneUserVideos = $videos
    ...
    .sort(sortByTitle);
  $: isDatapickerOpen = selectionVideoId && root && root.classList.contains('datapicker--open');
  ...
  function toggleDataPicker(id) {
    let open;
    Array.from(arguments).length === 2 && (open = arguments[1]);
...
>
  <Component>
    ...
    <List class="video-list" threeLine avatarList singleSelection bind:this={userVideoList} bind:selectedIndex>
            ...
            >
              {#if 'Administrator' === $session.role}
                <Button
                  ...
                  variant="unelevated"
                  on:click={() => toggleDataPicker(video.id, selectionVideoId != video.id ? true : undefined)}
                >
                  <Icon class="material-icons">
                    {isDatapickerOpen && selectionVideoId == video.id ? 'close' : 'insert_invitation'}
                  </Icon>
    ...
    <Component>
      <div slot="header">
        <div class="">
          ...
          <button on:click={() => toggleDataPicker(selectionVideoId, false)} class="button-close" variant="unelevated">
