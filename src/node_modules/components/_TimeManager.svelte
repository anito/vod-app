<script>
  ...
  const { getSnackbar, configAtts } = getContext("snackbar");
  ...
  onMount(() => {
    ...
    return () => root.classList.remove("timemanager--open");
  });
  ...
  async function saveTime(id, start, end) {
    ...
    if (res.success) {
      ...
      handleSuccess(res, currentUser, "Subscription updated");
    } else {
      handleError(res);
    }
  }
  ...
  async function toggle(video, e) {
    ...
    let added = (ids.length > userVideos.length && video) || false;
    ...
    if (res.success) {
      ...
      handleSuccess(
        res,
        currentUser,
        added ? "Video added" : "Video removed from schedule"
      );
    } else {
      handleError(res);
    }
...
<div
  ...
  <Component>
    <div slot="header">
      <h2>Video subscriptions <strong>({currentUser.name})</strong></h2>
    </div>
    {#if currentUser}
        ...
        <List class="video-list" threeLine avatarList singleSelection>
          {#each userVideos as video (video.id)}
            <SimpleVideoCard
              ...
              <IconButton class="close-action-button">
      ...
      {:else if userRole === 'Administrator'}
        <div class="flex flex-1 flex-col justify-center self-center">
          <div class="">Admins have full access to videos</div>
        </div>
      {:else}
        <div class="flex flex-1 flex-col justify-center self-center">
          <div class="">
...
{:else}
  <div class="grid-item videos" class:no-videos={!noneUserVideos.length}>
    <Component>
      ...
      {:else}
        <div class="flex flex-1 flex-col justify-center self-center">
          <div class="">No more videos available</div>
        </div>
      {/if}
    </Component>
  </div>
