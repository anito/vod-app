<script>
  ...
  $: userVideos =
    ($users[currentUserIndex] &&
      $users[currentUserIndex].videos.sort(sortByEndDate)) ||
    [];
  ...
  $: noneUserVideos = currentRole === ADMIN && $videos || $videos
    .filter(
      (v) =>
        !userVideos.find((uv) => {
          return (
            v.id === uv.id &&
            (!uv._joinData.end || new Date(uv._joinData.end) > new Date())
          );
        })
    )
    .sort(sortByTitle);
  $: isDatapickerOpen =
    selectionVideoId && root && root.classList.contains("datapicker--open");
  $: schedulingVideo =
    schedulingVideoId &&
    $videos.find((video) => video.id === schedulingVideoId);
  ...
  $: userIssues =
    ($infos.has(selectionUserId) &&
      $infos
        .get(selectionUserId)
        .params.filter((info) => info.type === "issue")) ||
    [];
              ...
              {#if ADMIN === $session.role}
                <Button
                  ...
                  on:click={() =>
                    toggleDataPicker(
                      video.id,
                      selectionVideoId != video.id ? true : undefined
                    )}
                >
