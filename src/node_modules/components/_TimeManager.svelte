<script>
  ...
  async function toggle(video, e) {
    ...
    if (res.success) {
      ...
      users.put(currentUser);
      if (isRemove) selectionVideoId = null;
    } else {
      this.error(res.status, res.data.message);
    }
  }
...
{:else}
  <div
    class="grid-item videos"
    class:no-videos={!noneUserVideos.length || userRole === 'Administrator'}>
    {#if noneUserVideos.length && userRole !== 'Administrator'}
      <List class="video-list" twoLine avatarList singleSelection>
