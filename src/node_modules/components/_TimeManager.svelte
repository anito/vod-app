<script>
  ...
  $: userVideos =
    ...
    $users[currentUserIndex].videos.sort(sortByTitle);
  ...
  $: noneUserVideos = $videos
    .filter((v) => userVideos && !userVideos.find((uv) => v.id === uv.id))
    .sort(sortByTitle);
...
<div
  ...
  class:no-videos={(userVideos && !userVideos.length) ||
...
>
  <Component>
    <div slot="header">
      <Header mdc h="5">
        {#if currentUser}
          <span><strong>{currentUser.name}</strong>`s Kurse</span>
        {/if}
      </Header>
    </div>
    <List
...
{:else}
  <div class="grid-item videos" class:no-videos={!noneUserVideos.length}>
    <Component>
      <div slot="header">
        <Header mdc h="5">Mehr Kurse</Header>
      </div>
    ...
    <List radioList>
      ...
      {#if !currentUser.token}
        <div class="mt-3">
          ...
          <div>
            Der Account des Benutzer scheint noch nicht freigeschaltet zu sein.
            Im Tab
            <i>Benutzerdaten</i>
