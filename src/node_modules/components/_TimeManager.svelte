<script>
	...
	$: setUserVideos(currentUserIndex); // make reactive in case video store has changed
	...
	function setUserVideos(idx) {
		let _videos = get(videos);
		let _users = get(users);
		let __userVideos;
		
		if (_users[idx] && (__userVideos = _users[idx].videos)) {
			userVideos = _videos.filter((v) => __userVideos.find( uv => uv.id === v.id));
		}
	}
	...
	async function toggle(video) {
		...
		if(ids.length < userVideos.length) {
...
{#if selectionVideoId}
	...
	<Fab disabled={!noneUserVideos.length} class="floating-fab" color="primary" on:click={() => selectionVideoId = null} extended>
