<script>
  ...
  $: userVideos = $users[currentUserIndex] && $users[currentUserIndex].videos.sort(sortByTitle);
  ...
  async function scheduleDialogCloseHandler(e) {
    if (e.detail.action === 'accept') {
      ...
      if (generateToken) {
        ...
        //   const res = await goto(
        //     `users/${currentUser.id}?tab=user&action=generate:token&path=${$page.path}`
        //   );
        // }, 100);
      }
    }
          ...
          {#each userVideos as video (video.id)}
            ...
            >
              {#if 'Administrator' === $session.role}
                ...
                <IconButton class="delete-action-button delete" on:click={(e) => openRemoveDialog(e, video)}>
    ...
    <List radioList>
      ...
      {#if !currentUser.token}
        <div class="mt-3">
          ...
          <div>
            Der Account des Benutzer scheint noch nicht freigeschaltet zu sein. Im Tab
            <i>Benutzerdaten</i>
            haben Sie die Möglichkeit einen Token zu generieren.
          </div>
        </div>
      {/if}
    </List>
