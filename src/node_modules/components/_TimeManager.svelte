<script>
  ...
  const timespanSelections = [
    ...
    { title: 'selbst auswählen', value: 'self' },
  ];
  ...
  $: userVideos = $users[currentUserIndex] && $users[currentUserIndex].videos.sort(sortByStartDate);
  ...
  $: noneUserVideos = $videos.filter((v) => userVideos && !userVideos.find((uv) => v.id === uv.id)).sort(sortByTitle);
  $: root && root.classList.toggle('datapicker--open', selectionVideoId);
  $: dialogVideo = dialogVideoId && $videos.find((video) => video.id === dialogVideoId);
      ...
      <Header mdc h="5">
        {#if currentUser}
          <div class="flex">
            ...
            <span class="uppercase flex-auto" style="font-weight: 400; text-align: right;"> | Gebuchte Kurse</span>
          </div>
        {/if}
      </Header>
    </div>
      ...
      {#if 'Administrator' !== currentRole}
          ...
          {#each userVideos as video (video.id)}
            ...
            >
              {#if 'Administrator' === $session.role}
                <Button class="close-action-button button-shaped-round flex self-center" variant="outlined">
        ...
        {:else if currentUser}
          <div class="flex flex-1 flex-col self-center text-center">
            <div class="my-10">
              {@html `Benutzer <strong>${currentUser.name}</strong> hat keine Videokurse abonniert`}
            </div>
          </div>
        {:else}
          <div class="flex flex-1 flex-col self-center text-center">
            <div class="my-10">
              {@html `Dieser Benutzer existiert nicht`}
            </div>
          </div>
        {/if}
    ...
    <Component>
      <div slot="header">
        <div class="">
          ...
          <button on:click={() => (selectionVideoId = null)} class="button-close" variant="outlined">
...
{:else}
  <div class="grid-item videos" class:no-videos={!noneUserVideos.length}>
    <Component>
      <div slot="header">
        <Header mdc h="5">
          <div class="uppercase" style="font-weight: 400; text-align: right;">Weitere Kurse</div></Header
        >
      </div>
      ...
      {:else}
        <div class="flex flex-1 flex-col self-center text-center">
          <div class="my-10">Keine weiteren Videokurse verfügbar</div>
        </div>
      {/if}
    </Component>
  </div>
...
>
    ...
    <List radioList>
      ...
      {#if !currentUser.token || isExpired || !currentUser.active}
        <div class="mt-3">
          ...
          <div>
            Der Account des Benutzers <strong>{currentUser.name}</strong> ist im Moment inaktiv. Über den Tab<i
              >Benutzerdaten</i
            >
            können Sie die folgenden Probleme beheben:
            <div>
              <ul class="reasons-list">
                {#if !currentUser.token}
                  <li>Der Accpunt verfügt über keinen gültigen Token</li>
                {/if}
                {#if isExpired}
                  <li>Der aktuelle Token ist ungültig</li>
                {/if}
                {#if !currentUser.active}
                  <li>Der Benutzer ist deaktiviert</li>
                {/if}
              </ul>
            </div>
          </div>
  ...
  <Actions>
    ...
    <Button action="accept" variant="outlined">
      <Label>Ok</Label>
    </Button>
  </Actions>
</Dialog>
...
>
  ...
  <Content id="event-content">
    Dieser Kurs hat ein aktives Zeitfenster.<br />
    <strong>{dialogVideo && dialogVideo.title}</strong>
    trotzdem entfernen?
  </Content>
  <Actions>
    <Button action="none">
    ...
    <Button action="approved" default use={[InitialFocus]} variant="outlined">
