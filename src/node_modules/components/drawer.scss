...
.drawer-container {
  .mdc-drawer {
    // Colors
    $mdc-drawer-title-ink-color: mdc-theme-prop-value(on-primary) !default;
    ...
    $mdc-drawer-divider-opacity: 0.12 !default;

    // Widths
    $mdc-drawer-width: 556px !default;
    ...
    $mdc-drawer-modal-scrim-opacity: 0.32 !default;
  ...
  .mdc-list-item--activated {
    &.error {
      // @include mdc-button-container-fill-color(error);Cz
      @include mdc-list-item-graphic-ink-color(error);
      @include mdc-list-item-primary-text-ink-color(error);
