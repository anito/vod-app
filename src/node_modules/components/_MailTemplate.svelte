<script>
  ...
  $: magicLinkLabel = ($settings.Site && $settings.Site['magic-link-label']) || 'Magic Link';
...
<div class="templates template-container grid main">
    ...
    <div class="content grid-item">
      ...
      {#if template['slug'] === 'magic-link'}
        <div class="template-data  flex justify-center mt-3 mb-3">
          <Button class={!template.data ? 'error' : ''} variant="unelevated" style="width: 80%; height: 80px;">
            <Label>{template.data ? magicLinkLabel : $_('text.missing-data')}</Label>
          </Button>
        </div>
      {/if}
    </div>
    <div class="after-content grid-item">
      <Textfield textarea label={$_('text.after-content')} bind:value={working['after-content']} style="width: 100%;" />
    </div>
  </div>
  <div class="grid footer main-grid-area">
    ...
    <div class="before-footer grid-item">
      <Textfield textarea label={$_('text.before-footer')} bind:value={working['before-footer']} style="width: 100%;" />
    </div>
    <div class="footer grid-item">
      <Textfield textarea label={$_('text.footer')} bind:value={working['footer']} style="width: 100%;" />
    </div>
    <div class="after-footer grid-item">
      <Textfield textarea label={$_('text.after-footer')} bind:value={working['after-footer']} style="width: 100%;" />
    </div>
  </div>
</div>
