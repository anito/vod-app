<script>
  ...
  $: magicLinkLabel =
    ($settings.Site && $settings.Site["magic-link-label"]) || "Magic Link";
    ...
    <div class="content grid-item">
      ...
      {#if template["slug"] === "magic-link"}
        <div class="template-data  flex justify-center mt-3 mb-3">
          <Button
            class={!template.data ? "error" : ""}
            variant="unelevated"
            style="width: 80%; height: 80px;"
          >
            <Label
              >{template.data ? magicLinkLabel : $_("text.missing-data")}</Label
            >
          </Button>
        </div>
      {/if}
