<script>
	...
	$: unreadInboxes = currentUser && $inboxes.filter((mail) => !mail._read).length;
	...
	function parseInbox() {
		...
		inboxData.map((mail) => {
			...
			items.push({
				id: mail.id,
				_from: getAddressees([mail._from]),
				_to: [user],
	...
	async function toggleRead(e) {
		...
		_read = e && e.detail._read != void 0 ? e.detail._read : !_selected._read;
		const res = await api.put(`inboxes/${_selected.id}`, { _read }, $session.user?.jwt);
		if (res.success) {
			selected = { ..._selected, _read };
