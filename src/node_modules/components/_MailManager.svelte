<script>
  ...
  import Fab, { Label, Icon as FabIcon } from "@smui/fab/styled";
  import Dialog, {
  ...
  $: _infos =
    ($infos.has(currentUser.id) && $infos.get(currentUser.id).params) || [];
  ...
  function getTemplateData(templates) {
    ...
    return { data };
    let isValid = currentUser.token && isValidToken();
    return {
      data:
        (isValid &&
          `http://${$page.host}/login?token=${
            ...
            {#if $session.role === "Administrator"}
              ...
              <div class="flex justify-between">
                <Subheader component={H6}
                  >{$_("text.email-templates")}</Subheader
                >
                <Group>
                  <IconButton
