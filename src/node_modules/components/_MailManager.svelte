<script>
  ...
  function saveEditable() {
    ...
    if (editor.value !== editor.node.innerText) saveTemplateName(editor.node.innerText);
  ...
  async function saveTemplateName(name) {
    const res = await api.put(`templates/${currentTemplate.id}`, { name }, $session.user && $session.user.token);
    if (res.success) {
      templates.put({ ...currentTemplate, name });
  ...
  <div class="grid-item mail">
    <div class="drawer-container">
      <Drawer variant="dismissible" bind:this={drawer} bind:open={drawerOpen}>
        ...
        <Content>
          <List>
            <Item href="" on:click={() => gotoActive('inboxes')} activated={active === 'inboxes'}>
            ...
            <Item href="" on:click={() => gotoActive('mails')} activated={active === 'mails'}>
            ...
            {#each $templates.sort(sortAZ) as template (template.id)}
              <Item
                on:click={() => gotoActive(templateStringFromSlug(template.slug))}
