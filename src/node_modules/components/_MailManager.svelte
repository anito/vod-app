<script>
  ...
  const templateSlugData = [
    {
      "magic-link": () => {
        return {
          validate: () => isValidToken() && getMagicLink(),
  ...
  $: currentTemplate =
    $templates.find((tmpl) => tmpl.slug === activeTemplate) || null;
  $: currentTemplate && $session.role === "Administrator"
    ? setFab("send-mail")
    : setFab("add-user");
  $: currentStore =
    activeMailbox === "inboxes"
      ? inboxes
      : activeMailbox === "sents"
      ? sents
      : null;
    ...
    const res = await api.post(
      "sents/add/",
      {
        ...
        template: {
          ...
          data: { ...templateData, href: templateData.validate() },
        },
      },
  ...
  function getTemplateData(slug) {
    let data;
    if (
      (data = templateSlugData.find((i) => i[slug])) &&
              ...
              {#each $templates.sort(sortAZProtected) as template (template.id)}
                ...
                >
                  ...
                  {#if !template.protected}
                    ...
                    {/if}
                  {/if}
                  {#if (tmplData = getTemplateData(template.slug)) && !tmplData.validate()}
                    <Meta>
