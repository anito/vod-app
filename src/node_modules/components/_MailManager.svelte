<script>
  ...
  import { fabs, users, sents, inboxes, templates } from "stores";
  ...
  const templateSlugData = [
    {
      "magic-link": () => {
        return {
          validate: () =>
            currentUser.active && isValidToken() && { href: getMagicLink() },
  ...
  $: currentUser =
    ((id) => $users.filter((usr) => usr.id === id))(selectionUserId)[0] ||
    $session.user;
  ...
  $: currentUser && parseInbox(currentUser);
  $: currentUser && parseSent(currentUser);
  $: templateSlug = $page.query.active || defaultActive;
  ...
  function createTemplatePath(active) {
    return `${validateUserPath($page.path)}${locationSearch({
      ...$page,
  ...
  function getMagicLink() {
    if (currentUser?.jwt) {
      return `https://${$page.host}/login?token=${currentUser.jwt}`;
    }
  }
</script>
