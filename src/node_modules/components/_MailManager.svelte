<script>
  ...
  import { parseQuery, slugify, locationSearch } from "utils";
  ...
  $: unreadInboxes =
    currentUser && $inboxes.filter((mail) => !mail.read).length;
  $: email = currentUser && currentUser.email;
  $: currentUser && getInbox(currentUser);
  $: currentUser && getSent(currentUser);
  $: $page.query.active && setActiveList($page.query.active);

  $: currentUser && (templateData = getTemplateData($templates).data);

  $: someTemplate = matchesTemplate(active);
  $: currentTemplate =
    $templates.find((tmpl) => tmpl.slug === someTemplate) || null;
  $: currentTemplate && $session.role === "Administrator"
    ? setFab("send-mail")
    : setFab("add-user");
  $: currentStore =
    active === "inboxes" ? inboxes : active === "sents" ? sents : null;
  $: validateActivePath = (active) => {
    return `${validatePath($page.path)}${locationSearch({
      ...$page,
  ...
  $: _infos =
    (currentUser &&
      $infos.has(currentUser.id) &&
      $infos.get(currentUser.id).params) ||
    [];
  ...
  onMount(() => {
    ...
    gotoActiveBox($page.query.active || defaultActive);
  ...
  async function sendMail() {
    const res = await api.post(
      "sents/add/",
      {
        ...
        ...working,
        template: { slug: currentTemplate.slug, data: templateData },
      },
    ...
    if (res.success) {
      res.data.map((mail) => {
        ...
        items.push({
          id: mail.id,
          from:
            (_users = $users.filter((user) => user.email === mail._from)) &&
            _users.length
              ? _users
              : [mail._from],
          to: [user],
  ...
  function setActiveList(value) {
    if (active != matchesTemplate(value) && canSave) {
  ...
  function getTemplateData(templates) {
    let _found, slug, data;
    for (var template in templates) {
      slug = templates[template]["slug"];
      if ((_found = slugValidator.find((val) => val[slug]))) {
        break;
      }
    }
    data =
      (_found && typeof _found[slug] === "function" && _found[slug]()) || false;
    return { data };
  }
  ...
  function isValidToken() {
    let expires;
    return (
      (expires = currentUser.expires) && expires * 1000 > +new Date().getTime()
    );
  }

  function generateName(name) {
    let newName =
      (name && name.concat($_("text.new-copy-label"))) || $_("text.new");
  ...
  function gotoActiveBox(active) {
    // fix falsy users and append mailbox path to url
    goto(validateActivePath(active));
  }
      ...
      <Drawer variant="dismissible" bind:this={drawer} bind:open={drawerOpen}>
        ...
        <Content>
          <List>
            <Item
              rel="prefetch"
              href={validateActivePath("inboxes")}
            ...
            {#if $session.role === "Administrator"}
                ...
                <Group>
                  <IconButton
                    ...
                    disabled={!currentTemplate ||
                      (currentTemplate && currentTemplate.protected)}
                  >
                    remove
                  </IconButton>
                  <IconButton
              ...
              {#each $templates.sort(sortAZProtected) as template (template.id)}
                <Item
                  rel="prefetch"
                  href={validateActivePath(
                    templateStringFromSlug(template.slug)
                  )}
                ...
                >
                  ...
                  >
                  <Text class={!template.protected && "editable"}
                    >{template.name}</Text
                  >
                  {#if !template.protected}
                    {#if editor.id === template.id}
                      ...
                      >
                      <Meta
                        on:click={(e) => saveEditable(e)}
                        class="material-icons-outlined edit">save</Meta
                      >
                    {:else}
                      <Meta
                    ...
                    {/if}
                  {/if}
                  {#if slugValidator.find((slug) => slug[template["slug"]]) && !templateData}
                    <Meta>
      ...
      <AppContent class="app-content">
        ...
        >
          <Icon class="material-icons"
            >{drawerOpen ? "menu_open" : "view_sidebar"}</Icon
          >
        </Button>
                ...
                <div class="grid-item grid-full">
                  <div class="empty-selection">
                    <span style="text-align: center;"
                      >{$_("text.empty-template-selection")}</span
                    >
                  </div>
                </div>
              {/if}
