...
<script>
  ...
  function saveEditable(e) {
    ...
    if (editor.value !== editor.node.innerText) saveTemplateName(editor.node.innerText, editor.id);
  ...
  async function saveTemplateName(name, id) {
    ...
    if (!template) return;
    const res = await api.put(`templates/${id}`, { name }, $session.user && $session.user.token);
    if (res.success) {
  ...
  function activePath(active) {
    return `${$page.path}${locationSearch({
      ...$page,
  ...
  <div class="grid-item mail">
    <div class="drawer-container">
      <Drawer variant="dismissible" bind:this={drawer} bind:open={drawerOpen}>
        ...
        <Content>
          <List>
            <Item rel="prefetch" href={activePath('inboxes')} activated={active === 'inboxes'}>
            ...
            <Item rel="prefetch" href={activePath('mails')} activated={active === 'mails'}>
