<script>
  ...
  function handlePlayPause(e) {
    !video.getAttribute("src") && video.setAttribute("src", video.dataset.src);
    // we have switched off preload, so must load first
    !duration && !video.promise && video.load();
    
    if (!video.promise || paused) {
      playhead && (video.currentTime = playhead);
  ...
  function handlePause(e) {
    ...
    dispatch("player:paused", { playhead });
  }
  ...
  function handlePictureInPicture(e) {
    ...
    else video && video.requestPictureInPicture().catch((e) => {});
  }
