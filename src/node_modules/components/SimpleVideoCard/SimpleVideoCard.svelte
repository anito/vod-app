<script>
  ...
  $: hasStarted = () =>
    compareAsc(new Date(), startDate) !== -1 ? true : false;
  $: timespan = () => {
    let start = hasStarted() ? new Date() : startDate;
    return (!emptyDate() && Math.abs(differenceInHours(start, endDate))) || 0;
  };
      ...
      {#if threeLine}
        <SecondaryText>
          <span class="text-xs text-inherit">
            ...
            <span>{timespan()} hrs left</span>
            <span>{!hasStarted() ? ' (not yet started)' : ''}</span>
          </span>
        </SecondaryText>
      {/if}
    </Text>
