<script>
  ...
  $: joinData = (_video = currentUser.videos.find((v) => v.id === video.id)) && _video._joinData;
  $: ((jData, _locale) => (
...
>
  ...
  <Text>
    ...
    {#if isUserVideo}
      <Set class="time-chip-set" chips={[1]}>
        <Chip class={(className = pending ? 'pending' : indefinite ? 'indefinite' : expired ? 'expired' : 'active')}>
          <Icon class="material-icons" leading>date_range</Icon>
          <Text>{readoutPeriod}</Text>
        </Chip>
      </Set>
    {:else}
      <SecondaryText>
        <span>{`${video.description || 'keine Beschreibung'}`}</span>
      </SecondaryText>
    {/if}
    {#if threeLine}
      <SecondaryText>
