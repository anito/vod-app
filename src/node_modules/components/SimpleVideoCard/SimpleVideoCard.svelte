<script>
  ...
  import { Icon } from '@smui/button/styled';
  import { Item, Graphic, Text, PrimaryText, SecondaryText } from '@smui/list/styled';
  import Chip, { Set, LeadingIcon } from '@smui/chips/styled';
  import { hasStarted, isExpired } from 'utils';
  import { getMedia } from 'utils/media';
  import { parseISO } from 'date-fns';
  import { localeFormat } from '@anito/site-kit';
  ...
  $: joinData = currentUser && (_video = currentUser.videos.find((v) => v.id === video.id)) && _video._joinData;
  $: ((jData, _locale) => (
    (startDate = jData && jData.start && parseISO(jData.start)),
    (endDate = (jData && jData.end && parseISO(jData.end)) || startDate),
    (indefinite = !(jData && jData.start)),
    (pending = startDate && !hasStarted(startDate)),
    (expired = (endDate && isExpired(endDate)) || false),
    (timerOff = expired || indefinite),
    (readoutPeriod =
      (startDate && `${localeFormat(startDate, dateFormat)} - ${localeFormat(endDate, dateFormat)}`) ||
...
>
  ...
  <Text>
    ...
    {#if isUserVideo}
      <Set class="time-chip-set" chips={[{ id: 0 }]} let:chip>
        <Chip
          {chip}
          class={(className = pending ? 'pending' : indefinite ? 'indefinite' : expired ? 'expired' : 'active')}
        >
  ...
  <span class="action-buttons flex flex-1 justify-end">
    <slot {expired} published={!indefinite} {unmanagable} />
  </span>
</Item>
