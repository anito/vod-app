<script>
  ...
  onMount(() => {
    if (forceOnExtend) {
      window.addEventListener("session:extended", forceVisible);
      forceVisible();
    }

    return () => {
      clearTimeout(timeout);
      window.removeEventListener("session:extended", forceVisible);
    };
  });

  Number.prototype.minDigits = function (
