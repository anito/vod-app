<script>
  import Day from '@anito/site-kit/components/DateRangePicker/components/Day.svelte';
...
<div class="row" role="row" on:nextMonth on:prevMonth>
  ...
  {/each}
  {#if weekNumbers}
    <small aria-label={`Week ${week.weekNumber}`} class="cell muted" title={`Week ${week.weekNumber}`}>
      {week.weekNumber}
    </small>
  {/if}
</div>
