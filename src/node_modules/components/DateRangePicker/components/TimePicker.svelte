<script>
  ...
  import { endOfDay, isAfter, isBefore, isSameHour, isSameMinute, isSameSecond, startOfDay } from 'date-fns';
  import { pad, roundDown } from '@anito/site-kit/components/DateRangePicker/utils';
  ...
  $: isHourOptionDisabled = (h) => {
    ...
    return (
      (!isSameHour(date, minDate) && isBefore(date, minDate)) || (!isSameHour(date, minDate) && isAfter(date, maxDate))
    );
  };
