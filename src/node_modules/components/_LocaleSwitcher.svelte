...
<span class="relative">
  ...
  <Menu bind:this={localeMenu} bind:anchorElement={localeMenuAnchor} anchor={false} anchorCorner="BOTTOM_LEFT">
    <List>
      {#each $locales as _locale}
        <Item on:SMUI:action={() => setLocale(_locale)}>
          <Text class={_locale === currentLocale && 'font-bold'}>{_locale.toUpperCase().slice(0, 2)}</Text>
        </Item>
      {/each}
    </List>
  </Menu>
