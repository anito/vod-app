<script>
    ...
    import { getExt, getMedia } from 'utils.js';
...
<Media aspectRatio="16x9">
    <MediaContent class="flex" style="display: flex; z-index: 0;">
        <div class="text-wrapper bg-gradient-to-b from-black" class:activeEditor class:isPlaying>
            <h2 class="mdc-typography--headline6 opacity-25" class:opacity-25={!media.title} style="margin: 0;">{media.title || 'No Title'}</h2>
            <h3 class="mdc-typography--subtitle2 opacity-25" class:opacity-25={!media.description} style="margin: 0;">{media.description || 'No Description'}</h3>
        </div>
