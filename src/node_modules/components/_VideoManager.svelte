			...
			<List class="user-video-list" twoLine avatarList singleSelection let:text>
				{#each userVideos as video (video.id)}
					<SimpleVideoCard
						...
						{selectionUserId}
					>
						<Button class="delete-action-button" on:click={(e) => toggle(video)}><Icon class="material-icons">remove_circle_outline</Icon><Label></Label></Button>
					</SimpleVideoCard>
				{/each}
			</List>
		{:else}
		...
		{#if noneUserVideos.length}
			{#each noneUserVideos as video (video.id)}
				<SimpleVideoCard
					...
					{selectionUserId}
				>
					<Button class="add-action-button" color="primary" on:click={(e) => toggle(video)}><Icon class="material-icons">add_circle_outline</Icon><Label></Label></Button>
				</SimpleVideoCard>
			{/each}
		{:else}
