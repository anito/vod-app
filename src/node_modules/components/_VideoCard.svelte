<script>
  ...
  import Card, { Content, PrimaryAction, Actions, ActionButtons, ActionIcons } from '@smui/card/styled';
  import Button, { Label } from '@smui/button/styled';
  import IconButton, { Icon } from '@smui/icon-button/styled';
  import Menu from '@smui/menu/styled';
  import MenuSurface, { Anchor } from '@smui/menu-surface/styled';
  import ImageList, { Item as ImageListItem, ImageAspectContainer, Image } from '@smui/image-list/styled';
  ...
  $: leftButton = isEditMode ? { label: $_('text.save'), icon: 'save' } : { label: $_('text.edit'), icon: 'edit' };
    ...
    <Content class="mdc-typography--body2">
      {#if 'Administrator' === $session.role}
        <div class="text-xs text-inherit">
          ...
          <span
            >{$_('text.uploaded-on', {
              values: { date: new Date(video.created).toLocaleDateString($locale, dateOptions) },
            })}</span
          >
        </div>
        <div class="opacity-25 text-xs text-inherit">
          <Icon class="material-icons">image</Icon>
          <div style="display: inline;">
            <span>Poster-ID:</span>
            <span>{video.image_id || $_('text.no-poster')}</span>
          </div>
        </div>
      {:else}
        <div class="">
          <div class="flex justify-between">
            <Icon class="material-icons" style="align-self: center;">date_range</Icon>
            <Label style="align-self: center; padding: 0 20px;">
              {@html $_('messages.video-available-until', {
                values: { date: new Date(matchingData.end).toLocaleDateString($locale) },
              })}
            </Label>
            <div class="date-button">
              <Button color="primary" variant="unelevated" class="button-shaped-round extended">
                <Label>
                  {@html $_('text.until-date', {
                    values: {
                      date: matchingData && new Date(matchingData.end).toLocaleDateString($locale, dateOptions),
                    },
                  })}
                </Label>
              </Button>
      ...
      <ActionIcons style="position: relative;">
        ...
        <Menu bind:this={cardMenu} on:MDCMenuSurface:opened={cardMenuOpenedHandler}>
          <List>
            ...
            <Separator />
            <Item disabled={!video.image_id} on:SMUI:action={() => dispatch('Video:removePoster', video.image_id)}>
                ...
                {:then src}
                  <ImageListItem>
                    <ImageAspectContainer>
                      <Image class="preview-image" on:click={() => dispatch('Video:selectPoster', image.id)} {src} />
                    </ImageAspectContainer>
                  </ImageListItem>
                {/await}
              {/each}
