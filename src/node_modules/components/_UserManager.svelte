<script>
  ...
  import Warnings from './_Warnings.svelte';
  ...
  $: ((user) => {
    ...
    expiresChipLabel = (expirationDate && `Token ${isExpired ? 'verfallen' : 'verfällt'}: ${expirationDate}`) || '';
                ...
                <div class="item">
                  {#if expirationDate}
                    <div class="chip-items">
                      <Chip>
                        <Icon class="material-icons" leading>{isExpired ? 'hourglass_empty' : 'hourglass_full'}</Icon>
                        <Text>{expiresChipLabel}</Text>
                      </Chip>
                    </div>
                  {/if}
                  <div class="chip-items">
                    <Warnings {selectionUserId} class="mt-1" />
                  </div>
                </div>
                <div class="item">
                  {#if token}
  ...
  <Content id="info-content">
    ...
    <div class="item">
      ...
      <details>
        ...
        <p>
          Benutzen Sie den Button
          <i>"{generateTokenText}"</i>, um einen <i>magischen Link</i> zu erzeugen. Dadurch wird auch automatisch der Account
          freigeschaltet. Die Gültigkeit des generierten Token richtet sich automatisch nach der Buchungsdauer des zuletzt
          auslaufenden Videos.
        </p>
        <p>
        ...
        <p>
          Denken Sie daran den Link an Ihre KlientInnen auszuhändigen, um den Zugang zu den gebuchten Inhalten zu
          ermöglichen.
        </p>
      </details>
      <details>
    ...
    <div class="item">
      ...
      Inhalte. Gehen Sie daher äusserst achtsam mit dem Token (Link) um und geben Sie den Link keinesfalls an Dritte weiter.
    </div>
    <div class="item">
      <h4>Gültigkeit</h4>
      Die Gültigkeit des Tokens richtet sich jeweils nach dem Buchungsende des spätesten Videos. Falls gerade keine Videos
      gebucht sind, aber dennoch ein neuer Token angefordert wird, hat dieser eine generelle Gültigkeit von 30 Tagen.
    </div>
  </Content>
  <Actions>
...
>
  ...
  <Content id="info-content">
    ...
    <div class="item">
      <i class="material-icons mdc-button__icon">warning</i>
      Eine Anmeldunng ist somit auch über das Anmeldeformular nicht mehr möglich.
    </div>
  </Content>
  <Actions>
    <Button action="none">
