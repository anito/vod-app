<script>
  ...
  $: ((user) => {
    ...
    expirationDate =
      (expires &&
        new Date(parseInt(expires) * 1000).toLocaleDateString(
          "de-DE",
          dateOptions
        )) ||
      null;
    ...
    <Component extended>
        ...
        <div class="">
          <Header h="5" mdc>
            <span>
              {#if currentUser}
                {currentUser.name}
              {:else}Neuen Klienten anlegen{/if}
            </span>
          </Header>
          <button
            on:click={() => goto('users')}
      ...
      <div class="flex flex-shrink">
        ...
        {#if currentUser && isAdmin}
          <div class="token-factory" class:has-magic-link={!!token_id}>
            <div class="main-info">
              ...
              <div class="item chip-items flex">
                <div
                  class="expires-label chip"
                  class:has-expired={hasExpired}
                  class:no-token={!token}>
                  <i
                    class="material-icons mdc-button__icon mr-3">{((hasExpired || !token) && 'announcement') || 'done'}</i>
                  {expiresChipLabel}
                </div>
                <div class="active-label chip" class:is-not-active={!active}>
                  <i
                    class="material-icons mdc-button__icon mr-3">{(!active && 'announcement') || 'done'}</i>
                  {activeChipLabel}
                </div>
              </div>
              <div class="item">
                ...
                <div class="item">
                  ...
                  <details>
                    ...
                    <p>
                      ...
                      <i>Magic Link</i>
                      klicken. Sie werden von Ihrem Account abgemeldet und
                      automatisch als
                      <strong>{currentUser.name}</strong>
                      angemeldet. Um später als
                      <strong>{$session.user.name}</strong>
                      weiter zu arbeiten, müssen Sie sich erneut als diese(r)
                      anmelden.
                    </p>
                  </details>
                  <details>
                    ...
                    <p>
                      Je nach Konfiguration Ihres Browsers, startet bei
                      Anklicken spezieller Email-Links ein Email Programm. Falls
                      dies bei Ihnen nicht der Fall sein sollte, lesen Sie den
                      nächsten Punkt.
                      <br />
                  ...
                  <details>
                    ...
                    <p>
                      ...
                      <i>Kopieren</i>
                      um den Link in Ihrer Zwischenablage zu speichern. Sie
                      können den Link anschliessend über den
  ...
  <Content id="info-content">
    ...
    <div class="item">
      <details>
        ...
        <p>
          Da der Token lediglich aus aneinandergereiten alphanumerischen Zeichen
          besteht, kann man ihn einfach als Parameter an einen Link "anhängen".
          Der Server der die angeforderte Seite bereitstellen soll, versucht den
          mitgelieferten Token zu entschlüsseln. Findet er einen geeigneten
          Benutzer dem er die Informationen zuordnen kann, wird dieser Benutzer
          angemeldet. Das übliche fehlerbehaftete Eintippen von
          Benutzernamen/Passwort entfällt also und es kann sofort losgelegt
          werden.
        </p>
      </details>
      <details>
    ...
    <div class="item">
      Die Gültigkeit des Tokens richtet sich jeweils nach dem Buchungsende des
      spätesten Videos. Falls gerade keine Videos gebucht sind, aber dennoch ein
      neuer Token angefordert wird, hat diser eine Gültigkeit von 30 Tagen.
    </div>
  </Content>
  <Actions>
...
<Dialog
  ...
  <Content id="info-content">
    <i class="material-icons mdc-button__icon">warning</i>
    <div class="item">
      Mit dieser Aktion wird der bisherige Token unbrauchbar. Besitzer des alten
      Tokens haben somit keine Möglichkeit mehr sich damit anzumelden!
    </div>
    <div class="item">
      Denken Sie daran,
      <strong>{currentUser.name}</strong>
      den neuen Token zu übermitteln.<br />Ein Login über das konventionelle
      "Benutzername/Passwort" Verfahren wird aber weiterhin möglich sein.
    </div>
  </Content>
  <Actions>
    <Button action="none">
...
<Dialog
  ...
  <Content id="info-content">
    <div class="item">
      Mit dieser Aktion wird der bisherige Token gelöscht und der Account
      gesperrt.
    </div>
    <div class="item">
      <i class="material-icons mdc-button__icon">warning</i>
