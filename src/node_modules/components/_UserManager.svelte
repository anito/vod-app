<script>
  ...
  $: userCan = ((userActions) =>
    [...actionsLookup].filter((s) => userActions.find((u) => s.action === u)))(
    actions
  );
  $: ((user) => {
  ...
  $: magicLink =
    (tokenVal && `http://${$page.host}/login?token=${tokenVal}`) || "";
  ...
  async function submit(event) {
    ...
    switch (mode) {
      ...
      case "del":
        if (
          !confirm($_("messages.permanently-remove-user", { values: { name } }))
        )
          return;
        res = await api.del(
          `users/${selectionUserId}?lang=${$locale}`,
  ...
  function copyToClipBoard(e) {
    if (textAreaMagicLink) {
      let didCopy,
        ...
        label =
          copyButton &&
          copyButton.getElementsByClassName("token-button-label").item(0);
              ...
              {#if selectedMode === "add" || selectedMode === "edit"}
                ...
                <div class="item">
                  ...
                  >
                    ...
                    >
                    <HelperText slot="helper" id="helper-text-standard-b"
                      >Ihr Name</HelperText
                    >
                  </Textfield>
                </div>
              ...
              {#if selectedMode === "del"}
                <div class="item">
                  <div class="alert" role="alert">
                    <div
                      class="bg-warning-500 text-white font-bold rounded-t px-4 py-2"
                    >
                      {$_("text.attention")}
                    </div>
                    <div
                ...
                <div class="button-group token-action-buttons flex mt-1 mb-3">
                  <Group>
                    <Button
                      ...
                      variant="raised"
                      on:click={() =>
                        proxyEvent("INFO:token:Generate", { open: !!token })}
                    >
                    ...
                    <Button
                      ...
                      variant="raised"
                      on:click={() =>
                        proxyEvent("INFO:token:Remove", { open: true })}
                    >
                ...
                <span>
                  ...
                  <a
                    ...
                    on:click|preventDefault={() =>
                      proxyEvent("INFO:open:InfoDialog")}
                  >
