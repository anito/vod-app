<script>
  ...
  $: uid = currentUser.id;
  $: (async (usr) => {
    let avatarId = usr.avatar && usr.avatar.id;
    if (!avatarId) {
      avatarSrc = defaultAvatar(usr);
      return;
    }

    let res = await getMedia("AVATAR", avatarId, user, {
      width: 100,
      height: 100,
      square: 1,
    });
      ...
      {#if selectionUserId}
        <div class="avatar-container" on:click={() => avatarMenu.setOpen(true)}>
          <div bind:this={avatarMenuAnchor} use:Anchor>
            <Avatar src={avatarSrc} user={currentUser} size="lg" />
            <Menu
