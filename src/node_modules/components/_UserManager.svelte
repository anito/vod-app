<script>
  ...
  async function generateToken() {
    ...
    if (res.success) {
      ...
      try {
        // validation message
        let message = res.data.errors.token._isUnique;
        configSnackbar(message || res.message);
      } catch (e) {
        console.log(e);
      }
      snackbar.open();
...
<Dialog
  ...
  <Content id="info-content">
    ...
    <div class="item">
      ...
      Benutzen Sie den Button
      <i>{removeTokenText}</i>
      um die Anmeldung über diesen Link zu verhindern. Sie können jederzeit
...
<Dialog
  ...
  <Content id="info-content">
    <i class="material-icons mdc-button__icon">warning</i>
    <div class="item">
      Sollte sich der Token geändert haben, denken Sie bitte daran, dem Nutzer
      <strong>{currentUser.name}</strong>
      den neuen Token zu übermitteln.<br />
      Es kann vorkommen, dass ein neu generierter Token identisch zum voherigen
      ist. Eine entsprechende Information wird am unteren Bildschirmrand
      eingeblendet.
    </div>
  </Content>
  <Actions>
