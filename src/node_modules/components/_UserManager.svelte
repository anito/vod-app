<script>
  ...
  import { users, flash, theme } from "stores";
  import { post, createRedirectSlug, proxyEvent, svg } from "utils";
  ...
  $: _group_id = ((usr) => (usr && usr.group.id) || "")(
    selectedMode !== "add" ? currentUser : false
  );
  ...
  async function deleteAvatar() {
    const res = await api.del(
      `avatars/${currentUser.avatar.id}?locale=${$locale}`,
      user?.jwt
    );
  ...
  async function submit(event) {
    ...
    switch (mode) {
      case "add":
        ...
        res = await api.post(`users/add?locale=${$locale}`, data, user?.jwt);
        break;
      case "edit":
          ...
          <div class="exception user-not-found">
            <div class="empty-selection no-user-selection">
              <span style="text-align: center;"
                >{$_("text.empty-user-selection")}</span
              >
            </div>
          </div>
        {:else}
