<script>
  ...
  const { getSnackbar, getSnackbarAtts } = getContext("snackbarUsers");
  let code;
  ...
  async function submit() {
    ...
    atts.message = (res.data && res.data.message) || res.statusText;
    ...
    if (res.success) {
      switch (selectedMode) {
        case "add":
          ...
          if (resUsers.success) {
            atts.path = `users/${res.data.id}?tab=user`;
            atts.action = "Goto new user";
      ...
      flash.update({ type: "error", message: atts.message });
      atts.path = `login${redirect($page)}`;
      atts.message += ". Redirecting...";
      snackbar.open();
    }
  }
