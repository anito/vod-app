<script>
  ...
  $: currentUser && (token = "");
  $: linkFromToken = (token && `<a href="/login?token=${token}"`) || "";
  ...
  async function getToken() {
    const res = await api.post(`token?token=${user.token}`, {
      id: currentUser.id,
    });
    if (res.success) {
  ...
  <Component extended>
    ...
    <div class="flex">
      ...
      <div class="token-factory">
        <div class="button-group flex justify-evenly mb-10 mt-1">
          <Button
            on:click={() => {
              getToken();
            }}
            variant="raised">
            <Label>Token gener.</Label>
          </Button>
          <Button on:click={() => (token = '')} variant="raised">
            <Label>Token löschen</Label>
          </Button>
        </div>
        ...
        {#if token}
          <div class="additional-info">
            <div class="item">
              <span>Mit einem Klick auf diesen </span>
              <a href="login?token={token}"><i>Geheimen Link</i></a>
              <span>werden Sie automatisch als
            ...
            <div class="item">
              Mit Rechtsklick -&gt;
              <i>Link kopieren</i>
              kann er nun z.B per Email an den betreffenden Benutzer
              weitergeleitet werden.
            </div>
            <div class="item">
              Gehen Sie auf jeden Fall achtsam mit diesen Informationen um. Sie
              sind sprichwörtlich der Schlüssel zum Account des Benutzers und
