<script>
	...
	async function deleteAvatar() {
		const res = await api.del(`avatars/${currentUser.avatar.id}?locale=${$locale}`, user?.jwt);
	...
	async function activateUser() {
		const res = await api.put(`users/${selectionUserId}?locale=${$locale}`, { active }, user?.jwt);
	...
	async function submit(event) {
		...
		switch (mode) {
			...
			case 'pass':
				...
				res = await api.put(`users/${selectionUserId}?locale=${$locale}`, data, user?.jwt);
				break;
			case 'del':
				if (!confirm($_('messages.permanently-remove-user', { values: { name } }))) return;
				res = await api.del(`users/${selectionUserId}?locale=${$locale}`, user?.jwt);
				break;
			default:
				return;
		}
			...
			if (res.success) {
				switch (selectedMode) {
					case 'add':
						...
						const resUsers = await api.get('users?limit=100', user?.jwt);
						if (resUsers.success) {
