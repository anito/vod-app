<script>
  ...
  import { post, getMedia, createRedirectSlug, proxyEvent } from 'utils';
  ...
  $: userCan = ((ua) => [...actionsLookup].filter((s) => ua.find((u) => s.action === u)))(actions);
  $: ((user) => {
    token = user && user.token;
    tokenVal = (token && token.token) || '';
    activeLabel = (active && `Benutzer deaktivieren`) || `Benutzer aktivieren`;
  })(currentUser);
  ...
  async function submit(event) {
    ...
    let mode = event instanceof SubmitEvent ? selectedMode : 'activate';
  ...
  function copyToClipBoard(e) {
    if (textAreaMagicLink) {
      ...
      if (didCopy) {
        label.innerText = dict.copySuccessButtonText;
        clearTimeout(copyTimeoutId);
        copyTimeoutId = setTimeout(() => (label.innerText = dict.copyButtonText), 4000);
      }
    }
  }
</script>
      ...
      <div class="flex flex-shrink">
            ...
            <div class="user-items">
                  ...
                  {#if isAdmin && selectedMode === 'edit'}
                    <div class="item">
                      <div class="ml-3" style="flex: 0.5;">
                        <FormField>
                          <Switch class="user-activation" bind:checked={active} on:change={() => submit()} />
              ...
              {#if selectedMode === 'add' || selectedMode === 'edit'}
                <div class="item">
                  ...
                  <HelperText validationMsg>{dict.invalidEmailMessage}</HelperText>
                </div>
                <div class="item">
                  <Textfield
                    withLeadingIcon
              ...
              {#if selectedMode === 'add' || selectedMode === 'pass'}
                <div class="item">
                  ...
                  <HelperText id="helper-password">{dict.passwordHint}</HelperText>
                </div>
                <div class="item">
                  <Textfield
                    withLeadingIcon
              ...
              {#if selectedMode === 'del'}
                <div class="item">
                  ...
                  <Button color="primary" variant="unelevated">
                    <Label>Ich habe verstanden</Label>
        ...
        {#if currentUser && isAdmin}
          <div class="table-wrapper">
            <div class="token-factory" class:has-magic-link={!!token}>
              <div class="main-info">
                <div class="button-group token-action-buttons flex mt-1 mb-3">
                  <Group>
                    ...
                    >
                      <Label class="token-button-label">{dict.removeTokenText}</Label>
                    </Button>
                  </Group>
                </div>
                <div class="item ">
                  ...
                  {#if token}
                    <div class="button-group magic-link-group token-action-buttons flex">
                      <Group style="max-width: 100%;">
                        <Button on:click={() => proxyEvent('INFO:token:Redirect')} variant="outlined">
                          <Icon class="material-icons">link</Icon>
                          <Label class="token-button-label">Magic Link</Label>
                        </Button>
                        <Button use={[(node) => (textAreaMagicLink = node)]} disabled={!token} variant="outlined">
                          <input style="width: 100%;" type="text" value={magicLink} />
                        </Button>
                        <Button disabled={!token} variant="unelevated" on:click={(e) => copyToClipBoard(e)}>
                          <Label class="token-button-label">
                            {dict.copyButtonText}
                          </Label>
                        </Button>
                  ...
                  <div class="item">
                    ...
                    <details>
                      ...
                      <p>
                        ...
                        <a
                          href="mailto:{currentUser.email}?subject=Ihr Zugangslink zu Ihren Online-Kursen&body=http://{$page.host}/login?token={tokenVal}&cc={user.email}"
                          target="_blank"
                        >
                          per Email an
                          {currentUser.name}</a
