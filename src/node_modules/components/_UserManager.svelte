<script>
  ...
  const generateMagicLinkTitle = "Token generieren";
  const removeTokenTitle = "Token entfernen";
  ...
  onMount(() => {
    ...
    return () => root.classList.remove("profiledata--open");
  });
  ...
  async function submit() {
    ...
    switch (selectedMode) {
      ...
      case "pass":
        data = password
          ? { email, name, group_id, password, token }
          : { email, name, group_id, token };
        res = await api.put(
    ...
    if (textAreaMagicLink) {
      ...
      if (didCopy) {
        ...
        copyTimeoutId = setTimeout(
          () => (label.innerText = copyButtonText),
          4000
        );
      }
    }
...
<style>
  ...
  .token-factory :global(button.magic-link) {
    --mdc-theme-primary: var(--prime-unused);
  }
  .token-factory .additional-info {
    overflow: auto;
  }
  .item-token-textfield :global(textarea) {
    font-size: 0.7rem;
    line-height: 1.2;
  }
...
<div class="grid-item user" style="height: 100%;">
  <Component extended>
    <div slot="header">
      <Header h="5" mdc>
        <span>
          {#if currentUser}{currentUser.name}{:else}Neuen Klienten anlegen{/if}
        </span>
      </Header>
    </div>
            ...
            <div class="select-item">
              <Select
                ...
                {#each userCan as can}
                  <Option
                    value={can.action}
                    selected={selectedMode === can.action}>
                    {can.name}
                  </Option>
                {/each}
              </Select>
        ...
        <div class="token-factory">
          <div class="main-info">
            <div class="button-group token-action-buttons flex mt-1 mb-3">
              <Group>
                ...
                <Button
                  ...
                  <Label class="token-button-label">{removeTokenTitle}</Label>
                </Button>
              </Group>
            </div>
            <div class="item">
              <span>
                ...
                <a
                  href="."
                  class="item"
                  on:click|preventDefault={() => openInfoDialog()}>
                  Wie funktioniert der Token?
                </a>
              </span>
            </div>
            <div class="item item-token-textfield">
              <Textfield
                textarea
                variant="filled"
                disabled
            ...
            <div class="item">
              {#if token_id}
                <div
                  class="button-group magic-link-group token-action-buttons flex">
                  <Group style="max-width: 100%;">
                    <Button
                      on:click={() => dispatch('openRedirectToUserDialog')}
                      variant="outlined">
                    ...
                    <Button
                      ...
                      <Label class="token-button-label">{copyButtonText}</Label>
                    </Button>
                  </Group>
                </div>
              {:else}
                <Button disabled style="min-width: 100%;">
                  <Label class="token-button-label">Zugang gesperrt</Label>
                </Button>
              {/if}
            </div>
          </div>
              ...
              <div class="item">
                ...
                <details>
                  ...
                  <p>
                    ...
                    <i>Magic Link</i>
                    klicken. Sie werden daraufhin von Ihrem Account abgemeldet
                    und auf einer neuen Seite automatisch als
                    <strong>{currentUser.name}</strong>
                    angemeldet. Um später hier weiter zu arbeiten, können Sie
                    das neu geöffnete Fenster wieder schliessen und zurück auf
                    diese Seite kehren. Da Sie abgemeldet wurden, müssen Sie
                ...
                <details>
                  ...
                  <p>
                    Je nach Konfiguration Ihres Browsers, kann durch Anklicken
                    spezieller Email-Links ein Email Programm gestartet werden:
                    <a
                ...
                <details>
                  ...
                  <p>
                    ...
                    <i>Kopieren</i>
                    um den Link in Ihrer Zwischenablage zu speichern. Sie können
                    den Link anschliessend über den
                    <i>Einfügen</i>-Befehl (<i>Strg-V</i>) in eine Email
                    einfügen und weiterleiten.
                  </p>
                </details>
              </div>
            {/if}
...
<Dialog
  ...
  <Content id="info-content">
    <div class="item">
      <i class="material-icons mdc-button__icon">info</i>
      Ein Token ist eine kryptische Zeichenkette mit verschlüsselten (Anmelde-)
      Informationen.<br />Hier ein Beispiel:

      <pre>
        eyJ0eXAiiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWiOiI0iIsImV4cCI6uGMTYwNzg2NTI5MX0.6UoD6T6v6jnbtwsoCudDL8LSr1HCW_ZqqVVXWrrY1kK4
      </pre>
    </div>
    <div class="item">
      <details>
        <summary>Wofür ist so ein Token da?</summary>
        <p>
          Da der Token letzten Endes lediglich aus aneinandergereiten
          alphanumerischen Zeichen besteht, kann man ihn einfach als Parameter
          an einen Link "anhängen". Der Server der die angeforderte Seite
          bereitstellen soll, versucht den mitgelieferten Token zu
          entschlüsseln. Findet er einen geeigneten Benutzer dem er die
          Informationen zuordnen kann, wird dieser Benutzer angemeldet. Das
          übliche fehlerbehaftete Eintippen von Benutzernamen/Passwort entfällt
          also und es kann sofort losgelegt werden.
        </p>
      </details>
      <details>
        <summary>Was muss ich tun?</summary>
        <p>
          Benutzen Sie den Button
          <i>"{generateMagicLinkTitle}"</i>, um einen
          <i>magischen Link</i>
          zu erzeugen. Dadurch wird auch automatisch der Account freigeschaltet.
          Um Ihren KlientInnen Zugang zu ihrem Account zu ermöglichen, müssen
          Sie nur noch den Link an sie aushändigen.
        </p>
      </details>
    </div>
    <div class="item">
      <i class="material-icons mdc-button__icon">info</i>
      Benutzen Sie den Button
      <i>{removeTokenTitle}</i>
      um z.B. bei Missbrauch oder Abhandenkommen des Tokens die Anmeldung durch
      Dritte über diesen Link zu verhindern. Sie können jederzeit einen neuen
      Token generieren.
    </div>
    <div class="item">
      <i class="material-icons mdc-button__icon">warning</i>
      Jeder im Besitz dieses Links verfügt automatisch über Zugriff auf
      gespeicherte Benutzerdaten, einschliesslich gebuchter Inhalte. Gehen Sie
      daher äusserst achtsam mit dem Token bzw. Link um und geben Sie ihn nicht
      an Dritte weiter.
    </div>
    <div class="item">Die Gültigkeit des Tokens beträgt 30 Tage</div>
  </Content>
  <Actions>
