<script>
  ...
  async function submit() {
    ...
    switch (selectedMode) {
      ...
      case "pass":
        ...
        break;
      case "del":
        if (confirm(`${name} will be permanently removed. Confirm pressing OK`))
    ...
    if (res.success) {
      switch (selectedMode) {
        case "add":
          ...
          if (resUsers.success) {
            redirectPath = `users/${res.data.id}?tab=user`;
      ...
      redirectPath = `login${redirect($page)}`;
  ...
  function handleClosed() {
    !action && redirectPath && goto(redirectPath);
  }
...
<Snackbar
  ...
  <Actions>
    {#if action}
      <Button on:click={goto(redirectPath)}>{action}</Button>
    {/if}
