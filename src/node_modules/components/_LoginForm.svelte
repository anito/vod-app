<script>
    ...
    let message;
    let timeout;
    let redirectPath = $page.query.redirect || '';
    ...
    function getRole(data) {
        let groups, group_id, idx;
        group_id = data && data.user && data.user.group_id;
        idx = data && (groups = data.groups) && groups.findIndex(itm=>itm.id==group_id)
        return groups[idx] && groups[idx].name;
    }
    ...
    async function submit( event ) {
		...
		if (res.success) {
            ...
            res.data && ($session.role = getRole(res.data));
            message += '. Redirecting...';
            snackbar.open();
		} else {
            snackbar.open();
...
<form on:submit|preventDefault={submit} method="post">
    <div class="grid login-grid">
        ...
        <div class="button-area flex">
            <Button class="flex-1" type="submit" variant="raised" disabled={!(email && password)}><Label>Login</Label></Button>
        </div>
    </div>
</form>
