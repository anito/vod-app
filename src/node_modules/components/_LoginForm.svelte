...
<script>
  ...
  import { formGuard as frozen } from "stores";
  ...
  const toplevel = process.env.NODE_ENV === "development" ? "dev" : "de";
  ...
  onMount(() => {
    ...
    (window.google && googleSignIn()) ||
      window.addEventListener("load", googleSignIn);
  ...
  async function submit() {
    ...
    await post(`auth/login?locale=${$locale}`, { email, password }).then(
      (res) => {
        let type;
        ...
        if (res.success) {
          resetFields();
          type = "success";
          proxyEvent("session:start", { ...res.data });
        } else {
          type = "warning";
  ...
  async function decodeJwtResponse(token) {
    ...
    await api
      .post(`users/google_login?locale=${$locale}`, {}, token)
      .then(async (res) => {
        if (res.success) {
