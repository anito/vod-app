<script>
  import "./_meta.scss";
  import { localeFormat, isToday } from "utils";
  import { onMount, onDestroy, createEventDispatcher } from "svelte";
  import UserGraphic from "./_UserGraphic.svelte";
  import { Item, Text, PrimaryText, SecondaryText } from "@smui/list/styled";
  ...
  onMount(() => {
    userItems =
      type === "inboxes" ? mail.from : type === "sents" ? mail.to : [];
  });
  onDestroy(() => {
    selected = null;
    userItems = [];
...
>
  ...
  <Text style="flex: 1;">
    ...
    >
      <span class="subject">{mail.subject || "--"}</span><span
        class="date-created">{created}</span
      >
    </SecondaryText>
  </Text>
</Item>
